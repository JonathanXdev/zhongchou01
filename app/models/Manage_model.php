...
class Manage_model extends CI_Model
{
	...
	public function add_manage()
	{
		...
		$data = array("uniacid" => $_W["uniacid"], "role_id" => $role_id, "group_id" => $group_id, "user_id" => $user_id, "name" => $name, "phone" => $phone, "email" => $email, "qq" => $qq, "addtime" => $time, "lasttime" => $time);
		if (!empty($managerow)) {
			$result = pdo_update("yhzc_manage", $data, array("id" => $id));
			if (!empty($result)) {
				return $id;
			}
		} else {
	...
	public function editrole()
	{
		...
		$data = array($name => $value, "uniacid" => $_W["uniacid"]);
		$result = pdo_update("yhzc_manage_role", $data, array("role_id" => $id));
		if (!empty($result)) {
	...
	public function editgroup()
	{
		...
		$id = $this->input->post("pk");
		$data = array($name => $value, "uniacid" => $_W["uniacid"]);
		$result = pdo_update("yhzc_manage_group", $data, array("group_id" => $id));
		if (!empty($result)) {
			die(json_encode(array("id" => 1, "msg" => "编辑成功")));
		}
	}
	public function addrole()
	{
		...
		$data = array("uniacid" => $_W["uniacid"], "role_name" => $role_name, "role" => $rolestr, "role_info" => $role_info);
	...
	public function addgroup()
	{
		...
		$data = array("uniacid" => $_W["uniacid"], "group_name" => $group_name, "group_info" => $group_info);
	...
	{
		$result = pdo_delete("yhzc_manage", array("id" => $id));
		if (!empty($result)) {
			return true;
		}
	}
	...
	{
		$result = pdo_delete("yhzc_manage_role", array("role_id" => $id));
		if (!empty($result)) {
			return true;
		}
	}
	...
	{
		$result = pdo_delete("yhzc_manage_group", array("group_id" => $id));
		if (!empty($result)) {
			return true;
		}
	}
